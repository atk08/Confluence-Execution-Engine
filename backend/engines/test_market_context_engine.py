"""
Tests for MarketContextEngine.
"""

from backend.engines.market_context_engine import MarketContextEngine
...
def test_market_context_engine():
    ...
    result = MarketContextEngine.analyze(context)

    assert result.score == 85.0
    assert result.bullish is True
    assert result.bearish is False
    assert result.trend_quality == 20.0
    assert result.structure_quality == 20.0
    assert result.bos_quality == 15.0
    assert result.choch_quality == 10.0